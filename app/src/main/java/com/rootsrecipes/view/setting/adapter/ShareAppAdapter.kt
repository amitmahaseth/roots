...
import com.rootsrecipes.databinding.ItemContactBinding
...
class ShareAppAdapter(
    ...
    private val contacts: List<ShareContact>,
    private var onItemClickListener: OnItemClickListener
) :
    RecyclerView.Adapter<ShareAppAdapter.BaseViewHolder>() {
    abstract class BaseViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView)

    private val sectionedContacts = contacts.groupBy { it.name[0].uppercase() }
        .toSortedMap()
    ...
    inner class ContactViewHolder(private val binding: ItemContactBinding) :
        ...
        fun bind(contact: ShareContact, isFirst: Boolean, isLast: Boolean, position: Int) {
            binding.apply {
                ...
                tvInvite.setOnClickListener { onItemClickListener.onItemClick(position , "contact_share") }
            }
        }
    }
    ...
    private fun getSectionForPosition(position: Int): String? {
        return sectionPositions.entries.find { (_, positions) ->
            position >= positions.headerPosition &&
                    position <= positions.lastItemPosition
        }?.key
    }
    ...
    override fun onBindViewHolder(holder: BaseViewHolder, position: Int) {
        ...
        when {
            ...
            sectionPosition != null -> {
                ...
                (holder as ContactViewHolder).bind(contact, isFirst, isLast , position)
