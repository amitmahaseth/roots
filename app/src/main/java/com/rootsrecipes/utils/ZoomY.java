...
public class ZoomY extends AppCompatImageView implements ScaleGestureDetector.OnScaleGestureListener {
    ...
    public ZoomY(Context context) {
        super(context);
        init(context, null);
    }

    public ZoomY(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public ZoomY(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray values = context.obtainStyledAttributes(attrs, R.styleable.ZoomageViewww);

        zoomable = values.getBoolean(R.styleable.ZoomageViewww_zoomage_zoomable, true);
        translatable = values.getBoolean(R.styleable.ZoomageViewww_zoomage_translatable, true);
        animateOnReset = values.getBoolean(R.styleable.ZoomageViewww_zoomage_animateOnReset, true);
        autoCenter = values.getBoolean(R.styleable.ZoomageViewww_zoomage_autoCenter, true);
        restrictBounds = values.getBoolean(R.styleable.ZoomageViewww_zoomage_restrictBounds, false);
        doubleTapToZoom = values.getBoolean(R.styleable.ZoomageViewww_zoomage_doubleTapToZoom, true);
        minScale = values.getFloat(R.styleable.ZoomageViewww_zoomage_minScale, MIN_SCALE);
        maxScale = values.getFloat(R.styleable.ZoomageViewww_zoomage_maxScale, MAX_SCALE);
        doubleTapToZoomScaleFactor = values.getFloat(R.styleable.ZoomageViewww_zoomage_doubleTapToZoomScaleFactor, 3);
        autoResetMode = AutoResetModee.Parser.fromInt(values.getInt(R.styleable.ZoomageViewww_zoomage_autoResetMode, AutoResetModee.UNDER));
    ...
    /**
     * Get the current {@link AutoResetModee} mode of the image. Default value is {@link AutoResetModee#UNDER}.
     *
     * @return the current {@link AutoResetModee} mode, one of {@link AutoResetModee#OVER OVER}, {@link AutoResetModee#UNDER UNDER},
     * {@link AutoResetModee#ALWAYS ALWAYS}, or {@link AutoResetModee#NEVER NEVER}
     */
    @AutoResetModee
    public int getAutoResetModee() {
        return autoResetMode;
    }

    /**
     * Set the {@link AutoResetModee} mode for the image.
     *
     * @param autoReset the desired mode, one of {@link AutoResetModee#OVER OVER}, {@link AutoResetModee#UNDER UNDER},
     *                  {@link AutoResetModee#ALWAYS ALWAYS}, or {@link AutoResetModee#NEVER NEVER}
     */
    public void setAutoResetModee(@AutoResetModee final int autoReset) {
    ...
    /**
     * Reset the image based on the specified {@link AutoResetModee} mode.
     */
    private void resetImage() {
        switch (autoResetMode) {
            case AutoResetModee.UNDER:
                if (matrixValues[Matrix.MSCALE_X] <= startValues[Matrix.MSCALE_X]) {
                    reset();
                } else {
                    center();
            ...
            case AutoResetModee.OVER:
                ...
                break;
            case AutoResetModee.ALWAYS:
                reset();
                break;
            case AutoResetModee.NEVER:
                center();
        }
    }
